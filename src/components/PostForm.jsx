    ...
    return <Card>
        <CardContent>
            <Typography variant='h6'>
               ...
               <TextField data-testid="post-field" multiline
               placeholder='post'
               value={post} onChange={({ target: {value}}) => setPost(value)}/>
               { error !== '' && <Alert severity='error' data-testid="alert-error">{error}</Alert>}
        </CardContent>
        <CardActions>
            <Button data-testid="post-button"variant='contained' onClick={handleCreatePost}>
